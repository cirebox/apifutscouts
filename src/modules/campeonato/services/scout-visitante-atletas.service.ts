...
export class ScoutVisitanteAtletasService {
  ...
  private readonly logger = new Logger(ScoutVisitanteAtletasService.name);
  ...
  async execute(): Promise<Futebol.ScoutJogador[]> {
    try {
      ...
      const retorno = [];
      await Promise.all(
        response.data.data.equipeVisitante.map((obj) => {
          const jogador = this.globalService.visitanteAtletas.filter(
