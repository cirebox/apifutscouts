...
export class CampeonatoController {
  constructor(
    private readonly campeonatoOpcoesService: CampeonatoOpcoesService,
  ...
  async options(): Promise<string[]> {
    return await this.campeonatoOpcoesService.execute();
  }
  ...
  @ApiParam({ name: "campeonato", type: "string", required: true })
  @ApiQuery({ name: "teste", type: "boolean", required: false })
  @Get("active/:campeonato")
  async active(
    @Param("campeonato") campeonato: string,
    @Query("teste") teste = "false",
  ): Promise<Futebol.OptionsPartida> {
    return await this.activeCampeonatoService.execute(campeonato, teste);
  }
